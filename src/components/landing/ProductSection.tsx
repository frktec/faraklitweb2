...
export function ProductSection() {
  return (
    <section id="urun" className="border-b border-ivory-200 bg-white">
      <div className="mx-auto max-w-8xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          <div>
            ...
            <h2 className="mt-6 max-w-[520px] font-serif text-[36px] font-normal leading-[1.1] tracking-[-0.02em] text-navy sm:text-[46px]">
              Günlük hukuk işlerinin tamamı, tek çalışma alanında.
            </h2>
            <p className="mt-6 max-w-[440px] text-[16px] leading-7 text-[#5a6473]">
              Farklı programlar, klasörler ve tablolar arasında gidip gelmeden; dosyadan süreye, araştırmadan dilekçeye kadar.
            </p>
          </div>

          <dl className="grid border-t border-ivory-200 sm:grid-cols-2">
            {capabilities.map(([number, title, body]) => (
              <div key={number} className="border-b border-ivory-200 py-7 sm:odd:pr-8 sm:even:pl-8">
                <dt className="flex items-baseline gap-4">
                  <span className="font-serif text-[14px] italic text-gold-600">{number}</span>
                  <span className="text-[17px] font-semibold text-navy">{title}</span>
                </dt>
                <dd className="mt-2 pl-9 text-[15px] leading-7 text-[#5f6977]">{body}</dd>
              </div>
            ))}
          </dl>
        </div>

        <div className="mt-20 grid gap-10 border-t border-ivory-200 pt-16 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          <div>
            <p className="text-[12px] font-semibold uppercase tracking-[0.24em] text-gold-600">Sesli asistan</p>
            <h3 className="mt-5 max-w-[440px] font-serif text-[30px] font-normal leading-[1.15] tracking-[-0.015em] text-navy sm:text-[36px]">
              Sorun, görev verin; Faraklit işi kayda geçirsin.
            </h3>
            <p className="mt-5 max-w-[420px] text-[16px] leading-7 text-[#5a6473]">
          ...
          <div className="divide-y divide-ivory-200 border-y border-ivory-200">
            {exchanges.map((item) => (
              <div key={item.question} className="grid gap-4 py-7 sm:grid-cols-[120px_1fr]">
                <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#8a8f98]">Avukat</p>
                <p className="font-serif text-[20px] italic leading-8 text-navy">“{item.question}”</p>
                <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-gold-600">Faraklit</p>
                <p className="text-[16px] leading-7 text-[#4a5566]">{item.answer}</p>
              </div>
            ))}
          </div>
        </div>
